...
def receive_clock_time(connector, address):
    while True:
        try:
            ...
            print(f"Error occurred while receiving clock time from {address}: {str(e)}")
            break
        time.sleep(5)
...
def connect_clients(master_server):
    while True:
        try:
            ...
            print(f"Error occurred while connecting clients: {str(e)}")
            break
        ...
        if len(client_data) > 0:
            ...
            for client_addr, client in client_data.items():
                try:
                    ...
                    print(f"Something went wrong while sending synchronized time through {client_addr}: {str(e)}")
        else:
